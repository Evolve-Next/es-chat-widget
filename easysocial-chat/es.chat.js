...
class ESChat {
  ...
  async init() {
    ...
    let t = await fetch(`https://api.easysocial.in/api/v1/whatsapp-widget/data?token=${this.token}`, { method: 'GET' });
    if (t.ok) {
      let o = await t.json();
  ...
  styleSetup() {
    let t = cEL('link');
    t.setAttribute('rel', 'stylesheet'), t.setAttribute('type', 'text/css'), t.setAttribute('href', 'https://cdn.easysocial.in/es.chat.min.css'), this.head.appendChild(t);
  }
  formContainer() {
  ...
  formButton() {
    (this.chatButtonEl = cEL('a')), (this.chatButtonEl.className = 'es-chat-iconButton'), (this.chatButtonEl.href = 'javascript:void(0);'), (this.chatButtonEl.ariaLabel = 'Chat With Us'), (this.chatButtonEl.tabIndex = '0'), this.formButtonVector();
  }
  injectButton() {
...
token ? new ESChat(token) : (console.error('A valid token is required'), console.info('Provide token either in "t" or "token"'));
